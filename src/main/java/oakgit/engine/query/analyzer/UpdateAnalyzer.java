...
public class UpdateAnalyzer implements QueryAnalyzer {
  ...
  private final Pattern UPDATE_PATTERN =
      Pattern.compile("update ([\\w_]+) set (.+?) where ID = \\? and MODCOUNT = \\?");
                ...
                for (String setExpression : setExpressions) {
                  ...
                  switch (updateExpression) {
                    ...
                    case "case when ? > MODIFIED then ? else MODIFIED end":
                      ...
                      placeHolderIndex++;
                      updateValue =
                          (Function<DocumentEntry, Long>)
                              entry ->
                                  modifiedCheck
                                          > (entry.getModified() != null ? entry.getModified() : 0L)
                                      ? modifiedValue
                                      : entry.getModified();
                      break;
                    case "2":
                    ...
                    case "DATA || CAST(? AS varchar(16384))":
                      ...
                      placeHolderIndex++;
                      updateValue =
                          (Function<DocumentEntry, byte[]>)
                              entry -> {
                                byte[] oldData =
                                    entry.getData() != null ? entry.getData() : new byte[0];
                                byte[] combined = new byte[oldData.length + newData.length];
                                System.arraycopy(oldData, 0, combined, 0, oldData.length);
                                System.arraycopy(
                                    newData, 0, combined, oldData.length, newData.length);
                                return combined;
                              };
                      break;
                    case "DSIZE + ?":
