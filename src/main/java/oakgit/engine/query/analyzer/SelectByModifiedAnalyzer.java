...
public class SelectByModifiedAnalyzer implements QueryAnalyzer {
  ...
  private static final Pattern SELECT_BY_MODIFIED_PATTERN =
      Pattern.compile(
          "select ([\\w\\s*,?=()]+?) from ([\\w_]+) where MODIFIED >= (?:'([^?]+)'|\\?) order by ID"
          ...
          result.setCommandSupplier(
              (placeholderData, selectionLimit) -> {
                ...
                return new SelectFromContainerByModifiedCommand<>(
                        tableName, Long.parseLong(modifiedValue), Integer.parseInt(limitValue))
