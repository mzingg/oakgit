...
public class SelectByRangeAnalyzer implements QueryAnalyzer {
  ...
  private final Pattern SELECT_BY_RANGE_PATTERN =
      Pattern.compile(
          "select ([\\w\\s*,]+?) from ([\\w_]+) where ID > \\? and ID < \\? order by ID(?: FETCH"
              + " FIRST (\\d+) ROWS ONLY)?");
        ...
        (result, matcher) -> {
          ...
          result.setCommandSupplier(
              (placeholderData, selectionLimit) ->
                  new SelectFromContainerByIdRangeCommand<>(
                          tableName,
