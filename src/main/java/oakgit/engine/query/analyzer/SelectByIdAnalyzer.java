...
public class SelectByIdAnalyzer implements QueryAnalyzer {
  ...
  private static final Pattern SELECT_BY_ID_PATTERN =
      Pattern.compile("select ([\\w\\s*,?=()]+?) from ([\\w_]+) where ID = (?:'([^?]+)'|\\?)");
  private static final Pattern CASE_PATTERN =
      Pattern.compile(
          "case when \\(MODCOUNT = \\? and MODIFIED = \\?\\) then null else (BDATA|DATA) end as"
              + " (?:BDATA|DATA)");
              ...
              (placeholderData, selectionLimit) -> {
                ...
                for (int i = 0; i < resultFieldList.size(); i++) {
                  ...
                  if (fieldMatcher.matches()) {
                    ...
                    resultFieldList.set(
                        i,
                        fieldMatcher.replaceAll(
                            String.format(
                                "case when (MODCOUNT = %d and MODIFIED = %d) then null else $1 end"
                                    + " as $1",
                                modCountCheck, modifiedCheck)));
                  }
                }
                String idByPlaceholder =
                    placeholderData.hasIndex(placeHolderIndex)
                        ? placeholderData.getString(placeHolderIndex)
                        : null;
                return new SelectFromContainerByIdCommand<>(
                        tableName, StringUtils.isNotBlank(idValue) ? idValue : idByPlaceholder)
