...
ARTIFACTS=(
  ...
  "com.adobe.granite.toggle.api|com.adobe.granite.toggle|com.adobe.granite.toggle.api|1"
...
SDK_SHORT_VERSION=$(echo "$SDK_FULL_VERSION" | grep -oP '^\d+\.\d+\.\d+')
...
if grep -q "<id>${PROFILE_ID}</id>" "$POM_FILE"; then
  ...
  awk -v profile_id="${PROFILE_ID}" -v new_block="${PROFILE_BLOCK}" '
    ...
    /<!-- AEM SDK .* \(generated by: nix run/ {
      # Peek: if next relevant line has our profile id, start skipping
      skip_comment = 1
      comment_line = $0
      next
    }
    skip_comment == 1 {
      # Check if this is the <profile> opening with our id coming
      if ($0 ~ "<profile>") {
        # Verify the id matches by reading ahead
        in_profile = 1
        depth = 1
        skip_comment = 0
        next
      } else {
        # False alarm — print the saved comment and this line
        print comment_line
        skip_comment = 0
      }
    }
      ...
      if ($0 ~ /<\/profile>/) {
        ...
        if (depth == 0) {
          if (confirmed) {
            ...
            print comment_line
            # We consumed lines we shouldnt have — this case shouldnt happen
            # with well-formed generated profiles
          }
          in_profile = 0
          confirmed = 0
          next
